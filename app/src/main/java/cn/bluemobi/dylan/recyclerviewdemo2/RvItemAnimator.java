...
/**
 * Created by cindy on 2018/1/29.
 * 改写DefaultItemAnimator 实现添加 删除动画从大变小
 */
...
//                .alpha(0)
        animation.setDuration(getRemoveDuration()).alpha(0).scaleX(0).scaleY(0) //修改坐标
...
//                ViewCompat.setScaleX(view, 1); //新加
//                ViewCompat.setScaleY(view, 1); //新加
...
//        mPendingAdditions.add(holder);
        resetAnimation(holder);
        ViewCompat.setAlpha(holder.itemView, 0);
        // 设置动画播放之前的状态，当前设置大小为零，即不可见状态 ,,主要就是设置了一下他的scaleX(1).scaleY(1)
    ...
    void animateAddImpl(final ViewHolder holder) {
        ...
        animation.setDuration(getAddDuration()).scaleX(1).scaleY(1).alpha(1) //新加
               . setListener(new VpaListenerAdapter() {
                    @Override
                    public void onAnimationStart(View view) {
